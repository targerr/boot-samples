 ...
 * @Classname SqlStateInterceptor
 * @Description mybatis拦截器。输出sql执行情况
 */
@Slf4j
...
public class SqlStateInterceptor implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        try {
            ...
            // 这个方法的总耗时
            log.info("\n\n ============= \nsql ----> {}\nuser ----> {}\nDataSources ----> {}\ncost ----> {}\n ============= \n", sql, uname, DsContextHolder.get(),cost);
        }
...
//            configuration = (Configuration) ReflectionUtils.getFieldVal(handler, "configuration", false);
            configuration = (Configuration) ReflectUtil.getFieldValue(handler, "configuration");
...
//            configuration = ((MappedStatement) ReflectionUtils.getFieldVal(paramHandler, "mappedStatement", false)).getConfiguration();
            configuration = ((MappedStatement) ReflectUtil.getFieldValue(paramHandler, "mappedStatement")).getConfiguration();
        }
